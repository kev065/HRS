...
post_args.add_argument('name', type=str, required=True, help='Session name is required')
...
class Sessions(Resource):
    ...
    def post(self):
        ...
        # Name validation
        assert re.match(r'Q[1-4]', data['name']), "Session name must be a quarter (Q1, Q2, Q3, Q4)"

        # Date validation
        start_date = datetime.strptime(data['start_date'], "%Y-%m-%d")
        end_date = datetime.strptime(data['end_date'], "%Y-%m-%d")
        assert start_date < end_date, "Start date must be earlier than end date"
        assert (end_date - start_date).days <= 92, "The session should represent a quarter of a year"
        ...
        new_session = Session(name=data['name'], start_date=start_date, end_date=end_date, goals=goal)
