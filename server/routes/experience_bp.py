...
post_args.add_argument('job_title', type=str, required=True, help='Job Title is required')
...
class Experiences(Resource):
    ...
    def post(self):
        ...
        new_experience=Experience(employee_id=employee_id,name=name,job_title=job_title,description=description,start_date=start_date,end_date=end_date)
...
class ExperienceById(Resource):
    def get(self, id):
        ...
        if not single_experience:
            abort(404, detail=f'Single Experience with  id {id} does not exist')
    ...
    def patch(self, id):
        ...
        if 'start_date' in data:
            data['start_date'] = datetime.strptime(data['start_date'], "%Y-%m-%dT%H:%M:%S.%f")
        if 'end_date' in data:
            data['end_date'] = datetime.strptime(data['end_date'], "%Y-%m-%dT%H:%M:%S.%f")
