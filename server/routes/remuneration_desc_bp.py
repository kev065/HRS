...
post_args.add_argument('remuneration_id', type=str, required=True, help='Remuneration ID is required')
post_args.add_argument('type', type=str, required=True, help='Type of Remuneration is required')
post_args.add_argument('name', type=str, required=True, help='Remuneration Name is required')
post_args.add_argument('description', type=str, required=True, help='Remuneration Description is required')
post_args.add_argument('amount', type=str, required=True,help='Remuneration Amount is required')
...
class RemunerationDescs(Resource):
    def get(self):
        ...
        result = remunerationDescriptionSchema.dump(remuneration_descs, many=True)
    ...
    def post(self):
        ...
        new_remuneration_descs =RemunerationDescription(remuneration_id=remuneration_id,type=type,name=name,description=description,amount=amount)
...
class RemunerationDescById(Resource):
    def get(self, id):
        single_remuneration_desc =RemunerationDescription.query.filter_by(id=id).first()

        if not single_remuneration_desc:
            abort(404, detail=f'Remuneration Description with  id {id} does not exist')
    ...
    def delete(self, id):
        ...
        if not  single_remuneration_desc:
            ...
            db.session.commit()
            response_body = {"message": "Remuneration Description successfully deleted"}
